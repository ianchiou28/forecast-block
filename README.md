...
在历史数据上验证模型性能（训练: 2022-2023年，测试: 2024年）：

**关键指标**：

```
训练数据：2022-01-01 至 2023-12-31 （24个月滚动窗口）
测试数据：2024-01-01 至 2024-12-31 （242个交易日）
...
│ 指标            │ 数值      │ 解释                  │
├─────────────────┼───────────┼──────────────────────┤
│ NDCG@5         │ 0.5886   │ 排序质量评分          │
│ 胜率            │ 56.20%   │ 盈利天数/总天数       │
│ 超额胜率        │ 68.18%   │ 跑赢市场天数/总天数   │
│ 日均收益        │ +0.52%   │ 预测板块平均收益      │
│ 日均超额收益    │ +0.47%   │ 超过市场均值的收益    │
│ 夏普比率        │ 3.13     │ 风险调整后收益        │
│ 最大回撤        │ -23.04%  │ 最大亏损幅度          │
│ 累计收益        │ 126.16%  │ 全年累计收益          │
│ 累计超额收益    │ 113.04%  │ 全年超额收益          │
└─────────────────────────────────────────────────────┘
```

**解释**：
- NDCG@5 = 0.5886 意味着模型的排序质量中等偏上（>0.5即有预测能力）
- 超额胜率 68.18% 意味着约2/3的交易日，预测板块能跑赢市场
- 日均超额收益 +0.47% 说明跟随预测结果每天能获得 0.47% 的额外收益
...
**关于收益的说明**：
- 上述数据基于2022-2023年训练、2024年测试的滚动回测
- 每月重新训练模型，确保适应最新市场规律
- 实盘中，收益会因以下因素降低 20%-40%：
  - **滑点成本**：买入/卖出时的价差
  - **手续费**：证券公司佣金（0.02%-0.03%）
  - **冲击成本**：大额买入导致的价格推升

**保守估计**：
- 实盘年化收益：70%-100%（经过费用和滑点调整）
- 夏普比率：1.5-2.0（仍属于优秀系统）
...
| 月份 | 累计收益 | 超额收益 | Top20%命中率 | NDCG |
|------|---------|---------|--------------|------|
| 2024-01 | -17.98% | +3.70% | 28.18% | 0.59 |
| 2024-02 | +24.86% | +12.47% | 37.33% | 0.43 |
| 2024-03 | +4.02% | -0.91% | 31.43% | 0.50 |
| 2024-04 | +0.77% | +2.81% | 30.00% | - |
| 2024-05 | +3.01% | +8.84% | 43.00% | - |
| 2024-06 | +0.08% | +5.38% | 37.89% | - |
| 2024-07 | +18.07% | +16.48% | 51.30% | 0.54 |
| 2024-08 | +6.53% | +11.62% | 43.64% | - |
| 2024-09 | +41.80% | +8.23% | 34.74% | 0.65 |
| 2024-10 | +12.96% | +15.44% | 50.00% | 0.60 |
| 2024-11 | +28.49% | +17.52% | 40.95% | 0.82 |
| 2024-12 | +3.54% | +11.46% | 45.45% | - |
...
效果数据（真实回测）：
├─ 训练[2022-2023] → 测试2024：NDCG=0.5886, 超额胜率=68.18%
├─ 累计收益：+126.16%，累计超额：+113.04%
├─ 夏普比率：3.13（风险调整后收益优秀）
└─ 结论：滚动训练策略在样本外测试中表现良好
...
**版本**：1.1 | **更新时间**：2026年1月4日 | **作者**：Ianchiou28

> ⚠️ **免责声明**：以上回测数据基于历史数据，不代表未来收益。投资有风险，入市需谨慎。
