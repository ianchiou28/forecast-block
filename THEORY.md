...
在历史数据上验证模型性能（2025年1月至2025年12月）：

**关键指标**：

```
时间范围：2025-01-02 至 2025-12-31 （1年历史数据）
测试周期：按月份划分，共12个独立的测试集
交易天数：243天
总预测次数：1215次
初始本金：¥10,000
...
│ 指标            │ 数值    │ 解释       │
├─────────────────┼─────────┼──────────┤
│ Hit Rate (Top1)│ 53.91%  │ 首选命中率 │
│ Hit Rate (Top3)│ 52.67%  │ 前三命中率 │
│ 整体命中率      │ 52.35%  │ Top5整体   │
│ 胜率            │ 55.14%  │ 盈利比例   │
│ 日均收益        │ +0.101% │ 涨幅优势   │
│ 夏普比率(Top1) │ 2.62    │ 风险调整收益│
│ 最大回撤        │ -11.97% │ 风险指标   │
└─────────────────────────────────────────┘
...
│ 策略            │ 最终资金  │ 年化收益 │ 夏普比率  │
├─────────────────┼───────────┼──────────┼──────────┤
│ Top-1策略       │ ¥16,543  │ 65.43%  │ 2.62     │
│ Top-5等权策略   │ ¥12,586  │ 25.86%  │ 1.43     │
└─────────────────────────────────────────────────────┘
```

**解释**：
- Hit Rate Top-1 = 53.91% 意味着首选板块命中率超过五成
- Hit Rate Top-3 = 52.67% 意味着预测的Top 3板块中，平均有52.67%会实现正收益
- 日均收益 +0.101% 说明跟随预测结果每天能获得 0.101% 的额外收益
- Top-1策略年化收益 65.43%，夏普比率 2.62，表现优于Top-5等权策略
...
```
年度统计（2025年回测数据，本金 ¥10,000）：
├─ 交易天数：243天（A股交易日）
├─ 盈利交易：134天
├─ 亏损交易：109天
├─ 胜率：55.14%
├─ Top-1策略最终资金：¥16,543（年化收益 +65.43%）
├─ Top-5等权策略最终资金：¥12,586（年化收益 +25.86%）
├─ 平均日收益：+0.101%
├─ **夏普比率（Top-1）**：2.62
└─ **最大回撤**：-11.97%

月度表现：
├─ 最佳月份：2025-08（平均收益 +0.49%，命中率 68.57%）
├─ 最差月份：2025-04（平均收益 -0.20%，命中率 51.43%）
└─ 月均命中率波动范围：39.05% ~ 68.57%
```

**现实期望值**：
- 上述数据是理想回测（不考虑滑点、手续费、冲击成本）
- 实盘中，由于以下因素，收益会降低 20%-30%：
...
**保守估计**：
- 实盘年化收益：25%-30%（经过费用和滑点调整）
- 夏普比率：1.5-1.8（仍属于优秀系统）
...
## 模型表现
- Hit Rate (Top-1): 50.21%
- Hit Rate (Top-3): 52.95%
- 整体命中率: 53.09%
- 日均收益: +0.155%
...
## 风险识别
- 最大回撤：-12.07%
- 最差月份：2025-01（命中率 36.67%）
- 模型过拟合风险：低
- 数据质量问题：无

## 改进建议
1. 考虑加入"板块涨幅限制"特征，避免高位追涨
2. 近期市场风格转变时，建议调整特征权重
3. 持续监控月度命中率波动
```
